...
class WebRTCPlayer {
    ...
    checkDependencies() {
        if (typeof SrsRtcPlayerAsync === 'undefined') {
            this.showError('SRS SDK not loaded. Please check your internet connection.');
    ...
    setupEventListeners() {
        ...
        // Zoom controls
        this.elements.zoomInBtn.addEventListener('click', () => this.zoom(ZOOM_STEP));
        this.elements.zoomOutBtn.addEventListener('click', () => this.zoom(-ZOOM_STEP));
        ...
        this.elements.video.addEventListener('play', () => this.updatePlayPauseIcon(true));
        this.elements.video.addEventListener('pause', () => this.updatePlayPauseIcon(false));
    ...
    async startPlaying() {
        ...
        try {
            ...
            const originalSetLocalDescription = this.sdk.pc.setLocalDescription.bind(this.sdk.pc);
            ...
            const originalSetRemoteDescription = this.sdk.pc.setRemoteDescription.bind(this.sdk.pc);
            this.sdk.pc.setRemoteDescription = async (desc) => {
                ...
                if (videoParams.codec) {
                    if (videoParams.codec.toLowerCase().includes('h265') ||
                        videoParams.codec.toLowerCase().includes('hevc')) {
                        this.state.codecType = 'H265/HEVC';
                    } else if (videoParams.codec.toLowerCase().includes('h264') ||
    ...
    async attemptReconnect() {
        ...
        this.updateConnectionStatus('connecting', `Reconnecting... (${this.state.reconnectAttempts}/${MAX_RECONNECT_ATTEMPTS})`);
    ...
    adjustVolume(delta) {
        const newValue = Math.max(0, Math.min(100, parseInt(this.elements.volumeSlider.value) + delta));
    ...
    // Zoom Controls
    zoom(delta) {
        const newScale = Math.max(ZOOM_MIN, Math.min(ZOOM_MAX, this.state.zoom.scale + delta));
    ...
    clampPan() {
        ...
        const maxX = Math.max(0, (rect.width * this.state.zoom.scale - rect.width) / 2);
        const maxY = Math.max(0, (rect.height * this.state.zoom.scale - rect.height) / 2);

        this.state.zoom.translateX = Math.min(maxX, Math.max(-maxX, this.state.zoom.translateX));
        this.state.zoom.translateY = Math.min(maxY, Math.max(-maxY, this.state.zoom.translateY));
    }
    ...
    startDebugMonitoring() {
        this.updateDebugInfo();
        this.debugInterval = setInterval(() => this.updateDebugInfo(), DEBUG_UPDATE_INTERVAL);
    }
    ...
    parseStats(stats) {
        ...
        stats.forEach(stat => {
            if (stat.type === 'inbound-rtp' && stat.mediaType === 'video') {
                ...
                const framesDecodedDelta = (stat.framesDecoded || 0) - (prevVideo.framesDecoded || 0);
                const framesDroppedDelta = (stat.framesDropped || 0) - (prevVideo.framesDropped || 0);
                const totalFramesDelta = framesDecodedDelta + framesDroppedDelta;
                const frameDropRate = totalFramesDelta > 0 ? (framesDroppedDelta / totalFramesDelta * 100) : 0;

                // Track key frame intervals
                const keyFramesDelta = (stat.keyFramesDecoded || 0) - (this.state.lastKeyFrameCount || 0);
                if (keyFramesDelta > 0) {
                    const currentTime = Date.now();
                    if (this.performanceMetrics.lastKeyFrameTime > 0) {
                        const keyFrameInterval = (currentTime - this.performanceMetrics.lastKeyFrameTime) / 1000;
                ...
                report.video = {
                    ...
                    bitrate: ((stat.bytesReceived - (prevVideo.bytesReceived || 0)) * 8) / timeDiff,
                    ...
                    avgKeyFrameInterval: this.performanceMetrics.keyFrameTimestamps.length > 0 ?
                        (this.performanceMetrics.keyFrameTimestamps.reduce((a, b) => a + b, 0) / this.performanceMetrics.keyFrameTimestamps.length).toFixed(2) : 'N/A',
                ...
                if (stat.framesDecoded > 0) {
                    report.video.avgDecodeTime = (stat.totalDecodeTime / stat.framesDecoded * 1000).toFixed(2);
                }

                // Track max FPS
                const currentFps = this.performanceMetrics.videoFrameCallbackFps || report.video.fps;
                ...
                report.audio = {
                    bytesReceived: stat.bytesReceived || 0,
                    bitrate: ((stat.bytesReceived - (prevAudio.bytesReceived || 0)) * 8) / timeDiff,
                    packetsLost: stat.packetsLost || 0,
                    packetsReceived: stat.packetsReceived || 0,
                    jitter: (stat.jitter * 1000) || 0,
                    ...
                    removedSamplesForAcceleration: stat.removedSamplesForAcceleration || 0,
                    insertedSamplesForDeceleration: stat.insertedSamplesForDeceleration || 0
                };

                this.lastStats.audio = stat;
            } else if (stat.type === 'candidate-pair' && stat.state === 'succeeded') {
                report.connection.rtt = stat.currentRoundTripTime ? (stat.currentRoundTripTime * 1000) : 0;
                ...
                report.connection.selectedCandidatePairChanges = stat.selectedCandidatePairChanges || 0;
    ...
    renderStatsTab(report) {
        ...
        if (report.video.resolution) {
            const packetLossRate = report.video.packetsReceived > 0 ?
                (report.video.packetsLost / (report.video.packetsLost + report.video.packetsReceived) * 100).toFixed(2) : 0;
            ...
            html += `
                <div class="debug-section">
                    <div class="debug-section-title">Video Statistics ${this.state.codecType ? `(${this.state.codecType})` : ''}</div>
                    <div class="debug-item">
                        <span class="debug-label">Resolution:</span>
                        <span class="debug-value">${report.video.resolution}</span>
                    </div>
                    <div class="debug-item">
                        <span class="debug-label">Actual FPS:</span>
                        <span class="debug-value ${this.getQualityClass(actualFps, 'fps')}">${actualFps.toFixed(1)} fps</span>
                    </div>
                    <div class="debug-item">
                        <span class="debug-label">WebRTC Stats FPS:</span>
                        <span class="debug-value">${report.video.fps.toFixed(1)} fps</span>
                    </div>
                    <div class="debug-item">
                        <span class="debug-label">Calculated FPS:</span>
                        <span class="debug-value">${report.video.measuredFps.toFixed(1)} fps</span>
                    </div>
                    <div class="debug-item">
                        <span class="debug-label">Max FPS Achieved:</span>
                        <span class="debug-value ${this.getQualityClass(this.performanceMetrics.maxFpsAchieved, 'fps')}">${this.performanceMetrics.maxFpsAchieved.toFixed(1)} fps</span>
                    </div>
                    <div class="debug-item">
                        <span class="debug-label">RAF Monitor FPS:</span>
                        <span class="debug-value ${this.getQualityClass(report.video.performanceFps, 'fps')}">${report.video.performanceFps.toFixed(1)} fps</span>
                    </div>
                    <div class="debug-item">
                        <span class="debug-label">Presented Frames:</span>
                        <span class="debug-value">${this.performanceMetrics.lastPresentedFrames || 'N/A'}</span>
                    </div>
                    <div class="debug-item">
                        <span class="debug-label">Bitrate:</span>
                        <span class="debug-value">${this.formatBitrate(report.video.bitrate)}</span>
                    </div>
                    <div class="debug-item">
                    ...
                    <div class="debug-item">
                        ...
                        <span class="debug-value">${report.video.avgDecodeTime || 'N/A'} ms</span>
                    </div>
                    <div class="debug-item">
                        <span class="debug-label">Total Frames Decoded:</span>
                        <span class="debug-value">${report.video.framesDecoded}</span>
                    </div>
                    <div class="debug-item">
                        <span class="debug-label">Total Frames Dropped:</span>
                        <span class="debug-value">${report.video.framesDropped}</span>
                    </div>
                    <div class="debug-item">
                        <span class="debug-label">Frame Drop Rate:</span>
                        <span class="debug-value ${report.video.frameDropRate > 5 ? 'bad' : report.video.frameDropRate > 1 ? 'warning' : 'good'}">${report.video.frameDropRate.toFixed(2)}%</span>
                    </div>
                    <div class="debug-item">
                        <span class="debug-label">Key Frames:</span>
                        <span class="debug-value">${report.video.keyFramesDecoded}</span>
                    </div>
                    <div class="debug-item">
                        <span class="debug-label">Avg Key Frame Interval:</span>
                        <span class="debug-value">${report.video.avgKeyFrameInterval} s</span>
                    </div>
                    <div class="debug-item">
                        <span class="debug-label">Packet Loss:</span>
                        <span class="debug-value ${this.getQualityClass(packetLossRate, 'packetLoss')}">${packetLossRate}%</span>
                    </div>
                    <div class="debug-item">
                        <span class="debug-label">Jitter:</span>
                        <span class="debug-value ${this.getQualityClass(report.video.jitter, 'jitter')}">${report.video.jitter.toFixed(1)} ms</span>
                    </div>
                    <div class="debug-item">
                        <span class="debug-label">PLI/NACK/FIR:</span>
                        <span class="debug-value">${report.video.pliCount}/${report.video.nackCount}/${report.video.firCount}</span>
                    </div>
            ...
            if (actualFps < 15) {
                html += `
                    <div class="perf-warning">
                        ...
                        <div>Actual FPS: ${actualFps.toFixed(1)} (from requestVideoFrameCallback)</div>
                        ...
                        <ul style="margin: 8px 0 0 20px; padding: 0;">
                            ...
                            <li>Network congestion (check jitter: ${report.video.jitter.toFixed(1)}ms)</li>
                        ...
                        <ul style="margin: 4px 0 0 20px; padding: 0;">
                            <li>PLI Count: ${report.video.pliCount} (Picture Loss Indication)</li>
                            <li>NACK Count: ${report.video.nackCount} (Negative Acknowledgment)</li>
                            <li>FIR Count: ${report.video.firCount} (Full Intra Request)</li>
                            <li>Key Frame Interval: ${report.video.avgKeyFrameInterval}s</li>
                        </ul>
                    </div>
                `;
            }
        ...
        if (report.audio.bytesReceived !== undefined) {
            const audioPacketLossRate = report.audio.packetsReceived > 0 ?
                (report.audio.packetsLost / (report.audio.packetsLost + report.audio.packetsReceived) * 100).toFixed(2) : 0;
            ...
            html += `
                <div class="debug-section">
                    ...
                    <div class="debug-item">
                        <span class="debug-label">Bitrate:</span>
                        <span class="debug-value">${this.formatBitrate(report.audio.bitrate)}</span>
                    </div>
                    <div class="debug-item">
                        <span class="debug-label">Audio Level:</span>
                        <span class="debug-value">${(report.audio.audioLevel * 100).toFixed(1)}%</span>
                    </div>
                    <div class="debug-item">
                        <span class="debug-label">Packet Loss:</span>
                        <span class="debug-value ${this.getQualityClass(audioPacketLossRate, 'packetLoss')}">${audioPacketLossRate}%</span>
                    </div>
                    <div class="debug-item">
                        <span class="debug-label">Jitter:</span>
                        <span class="debug-value ${this.getQualityClass(report.audio.jitter, 'jitter')}">${report.audio.jitter.toFixed(1)} ms</span>
                    </div>
                    <div class="debug-item">
                        <span class="debug-label">Concealed Samples:</span>
                        <span class="debug-value">${report.audio.concealedSamples}</span>
                    </div>
                    <div class="debug-item">
                        <span class="debug-label">Concealment Events:</span>
                        <span class="debug-value">${report.audio.concealmentEvents}</span>
                    </div>
        ...
        requestAnimationFrame(() => {
            const fpsData = this.statsHistory.videoFrameCallbackFps.length > 0 ?
                this.statsHistory.videoFrameCallbackFps : this.statsHistory.fps;
            this.drawGraph('fps-graph', fpsData, 70, 'Actual FPS');
            this.drawGraph('audio-graph', this.statsHistory.bitrate, Math.max(...this.statsHistory.bitrate, 1000000), 'Audio Bitrate');
        });
    }
    ...
    renderConnectionTab(report) {
        ...
        if (report.connection.dtlsState) {
            const totalBytes = (report.connection.bytesSent || 0) + (report.connection.bytesReceived || 0);

            html += `
                <div class="debug-item">
                    <span class="debug-label">Connection State:</span>
                    <span class="debug-value">${this.sdk.pc.connectionState}</span>
                </div>
                <div class="debug-item">
                    <span class="debug-label">DTLS State:</span>
                    <span class="debug-value">${report.connection.dtlsState}</span>
                </div>
                <div class="debug-item">
                    <span class="debug-label">ICE State:</span>
                    <span class="debug-value">${report.connection.iceState}</span>
                </div>
                <div class="debug-item">
                    <span class="debug-label">Protocol:</span>
                    <span class="debug-value">${report.connection.protocol || 'N/A'}</span>
                </div>
                <div class="debug-item">
                    <span class="debug-label">RTT:</span>
                    <span class="debug-value ${this.getQualityClass(report.connection.rtt, 'rtt')}">${report.connection.rtt ? report.connection.rtt.toFixed(1) + ' ms' : 'N/A'}</span>
                </div>
                <div class="debug-item">
                    <span class="debug-label">Available Bandwidth:</span>
                    <span class="debug-value">${this.formatBitrate(report.connection.availableBitrate)}</span>
                </div>
                <div class="debug-item">
                    <span class="debug-label">Local Candidate:</span>
                    <span class="debug-value">${report.connection.localCandidateType || 'N/A'}</span>
                </div>
                <div class="debug-item">
                    <span class="debug-label">Remote Candidate:</span>
                    <span class="debug-value">${report.connection.remoteCandidateType || 'N/A'}</span>
                </div>
                <div class="debug-item">
                    <span class="debug-label">Total Data Transfer:</span>
                    <span class="debug-value">${this.formatBytes(totalBytes)}</span>
                </div>
                <div class="debug-item">
                    <span class="debug-label">Packets Received:</span>
                    <span class="debug-value">${report.connection.packetsReceived || 0}</span>
                </div>
                <div class="debug-item">
                    <span class="debug-label">Candidate Pair Changes:</span>
                    <span class="debug-value">${report.connection.selectedCandidatePairChanges}</span>
                </div>
                <div class="debug-item">
                    <span class="debug-label">STUN Requests Sent:</span>
                    <span class="debug-value">${report.connection.requestsSent || 0}</span>
                </div>
                <div class="debug-item">
                    <span class="debug-label">STUN Responses Received:</span>
                    <span class="debug-value">${report.connection.responsesReceived || 0}</span>
                </div>
            `;
        }
        ...
        if (report.candidates.length > 0) {
            ...
            const localCandidates = report.candidates.filter(c => c.type === 'local-candidate');
            const remoteCandidates = report.candidates.filter(c => c.type === 'remote-candidate');
            ...
            html += `
                <div class="debug-section">
                    ...
                    <div class="debug-item">
                        ...
                        <span class="debug-value">${videoParams.tierFlag === 1 ? 'High' : 'Main'}</span>
                    </div>
                    ` : ''}
                    ${videoParams.txMode ? `
                    <div class="debug-item">
        ...
        // Draw graphs
        requestAnimationFrame(() => {
            this.drawGraph('rtt-graph', this.statsHistory.rtt, Math.max(...this.statsHistory.rtt, 200), 'RTT (ms)');
        });
    }
    ...
    renderMediaTab(report) {
        ...
        if (report.codecs.video) {
            html += `
                <div class="debug-item">
                    ...
                    <span class="debug-value">${report.codecs.video} ${this.state.codecType ? `(${this.state.codecType})` : ''}</span>
                </div>
                <div class="debug-item">
                    <span class="debug-label">Implementation:</span>
                    <span class="debug-value">${report.codecs.videoImplementation || 'N/A'}</span>
                </div>
                <div class="debug-item">
                    <span class="debug-label">Clock Rate:</span>
                    <span class="debug-value">${report.codecs.videoClockRate} Hz</span>
                </div>
                <div class="debug-item">
                    <span class="debug-label">Payload Type:</span>
                    <span class="debug-value">${report.codecs.videoPayloadType}</span>
                </div>
            `;
        }
        ...
        if (report.codecs.audio) {
            html += `
                ...
                <div class="debug-item">
                    <span class="debug-label">Clock Rate:</span>
                    <span class="debug-value">${report.codecs.audioClockRate} Hz</span>
                </div>
                <div class="debug-item">
                    <span class="debug-label">Channels:</span>
                    <span class="debug-value">${report.codecs.audioChannels || 'N/A'}</span>
                </div>
                <div class="debug-item">
                    <span class="debug-label">Payload Type:</span>
                    <span class="debug-value">${report.codecs.audioPayloadType}</span>
                </div>
            `;
        }
        ...
        html += `
            <div class="debug-section">
                ...
                <div class="debug-item">
                    ...
                    <span class="debug-value">${['HAVE_NOTHING', 'HAVE_METADATA', 'HAVE_CURRENT_DATA', 'HAVE_FUTURE_DATA', 'HAVE_ENOUGH_DATA'][this.elements.video.readyState]}</span>
                </div>
                <div class="debug-item">
                    <span class="debug-label">Network State:</span>
                    <span class="debug-value">${['NETWORK_EMPTY', 'NETWORK_IDLE', 'NETWORK_LOADING', 'NETWORK_NO_SOURCE'][this.elements.video.networkState]}</span>
                </div>
                <div class="debug-item">
                    <span class="debug-label">Video Width:</span>
                    <span class="debug-value">${this.elements.video.videoWidth}</span>
                </div>
                <div class="debug-item">
                    <span class="debug-label">Video Height:</span>
                    <span class="debug-value">${this.elements.video.videoHeight}</span>
                </div>
                <div class="debug-item">
                    <span class="debug-label">Display Size:</span>
                    <span class="debug-value">${this.elements.video.clientWidth}x${this.elements.video.clientHeight}</span>
                </div>
                <div class="debug-item">
                    <span class="debug-label">Buffered:</span>
                    <span class="debug-value">${this.elements.video.buffered.length > 0 ? this.elements.video.buffered.end(0).toFixed(1) + 's' : '0s'}</span>
                </div>
                <div class="debug-item">
                    <span class="debug-label">Current Time:</span>
                    <span class="debug-value">${this.elements.video.currentTime.toFixed(1)}s</span>
                </div>
                <div class="debug-item">
                    <span class="debug-label">Volume:</span>
                    <span class="debug-value">${Math.round(this.elements.video.volume * 100)}%</span>
                </div>
                <div class="debug-item">
                    <span class="debug-label">Muted:</span>
                    <span class="debug-value">${this.elements.video.muted ? 'Yes' : 'No'}</span>
                </div>
                <div class="debug-item">
                    <span class="debug-label">Playback Rate:</span>
                    <span class="debug-value">${this.elements.video.playbackRate}x</span>
                </div>
            </div>
        `;
        ...
        html += `
            <div class="debug-section">
                ...
                <div class="debug-item">
                    ...
                    <span class="debug-value ${('requestVideoFrameCallback' in HTMLVideoElement.prototype) ? 'good' : 'bad'}">
                        ${('requestVideoFrameCallback' in HTMLVideoElement.prototype) ? 'Supported' : 'Not Supported'}
                    </span>
                </div>
                <div class="debug-item">
                    <span class="debug-label">User Agent:</span>
                    <span class="debug-value" style="font-size: 11px; word-break: break-all;">${navigator.userAgent}</span>
                </div>
            </div>
        `;
            ...
            report.streams.forEach((stream, index) => {
                html += `
                    <div class="debug-item">
                        ...
                        <span class="debug-value" style="font-size: 11px;">${stream.id}</span>
                    </div>
                `;
            });
            html += `</div>`;
    ...
    updateStatsHistory(report) {
        ...
        if (report.video.videoFrameCallbackFps !== undefined) {
            this.statsHistory.videoFrameCallbackFps.push(report.video.videoFrameCallbackFps);
    ...
    drawGraph(canvasId, data, maxValue, label) {
        ...
        ctx.strokeStyle = currentValue >= 55 ? '#4CAF50' : currentValue >= 30 ? '#FFC107' : '#F44336';
        ctx.lineWidth = 2;
        ctx.beginPath();
    ...
    exportDebugData() {
        ...
        const data = {
            ...
            playerState: {
                ...
                buffered: this.elements.video.buffered.length > 0 ? this.elements.video.buffered.end(0) : 0,
    ...
    loadPlayerSettings() {
        try {
            const settings = JSON.parse(localStorage.getItem('webrtc-player-settings') || '{}');
